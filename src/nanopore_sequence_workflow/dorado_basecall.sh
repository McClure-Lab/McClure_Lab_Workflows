...
usage() {
    echo "Usage: bash src/nanopore_sequence_workflow/dorado_basecall.sh --pod5 POD5_FILE_OR_DIR --output-dir DIR --log-file LOG --model MODEL --device DEVICE --min-qscore N [--max-reads N] [--output-format fastq|bam|bam_basecalled] [--reference FASTA] [--emit-moves yes|no] [--mm2-opts OPTS] [--kit-name KIT] [--barcode-mode none|demux] [--barcode-output-dir DIR] [--modified-bases MODEL_OR_CODE]"
    echo
    echo "bam produces an aligned BAM and requires --reference."
    echo "bam_basecalled produces Dorado's native unaligned BAM (no --reference used)."
    echo "bam_basecalled is what the parent workflow's --output-format both uses internally."
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --model|--dorado-model) DORADO_MODEL="$2"; shift 2 ;;
...
case "$OUTPUT_FORMAT" in
    fastq|bam|bam_basecalled) ;;
    *)
        echo "[ERROR] Invalid output format: $OUTPUT_FORMAT"
        echo "[ERROR] Expected fastq, bam, or bam_basecalled."
        exit 1
        ;;
esac
...
# A sequencing-kit name is required for Dorado to identify which barcode
# arrangement should be used during demultiplexing.
if [[ "$BARCODE_MODE" == "demux" && -z "$KIT_NAME" ]]; then
    echo "[ERROR] --kit-name is required when --barcode-mode demux is used."
    exit 1
fi

# Create the output directory and the parent directory containing the log file.
...
case "$OUTPUT_FORMAT" in
    fastq)
        ...
        ;;
    bam)
        # --reference makes Dorado basecall and align in one step, producing
        # an aligned BAM.
        ...
        ;;
    bam_basecalled)
        # No --reference and no --emit-fastq: Dorado writes its native
        # unaligned BAM. The parent workflow aligns this separately when
        # producing --output-format both, so the (comparatively expensive)
        # GPU basecalling step only has to run once.
        if [[ "$EMIT_MOVES" == "yes" ]]; then
            DORADO_ARGS+=(--emit-moves)
        fi
        ;;
...
if [[ "$BARCODE_MODE" == "demux" ]]; then
    ...
    # Rename Dorado's barcode outputs so every filename begins with the
    # original POD5 sample prefix and job ID.
...
if [[ "$OUTPUT_FORMAT" == "fastq" ]]; then
...
else
    {
        ...
        echo "  BASECALL QC"
        echo "========================================="
        if [[ "$OUTPUT_FORMAT" == "bam_basecalled" ]]; then
            echo "Dorado produced unaligned (basecalled) BAM output."
        else
...
# Print the primary output path to standard output so a parent workflow
# can capture the generated filename.
echo "$OUTPUT_FILE"
